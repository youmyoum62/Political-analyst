...
export function getRisingPoliticians(politicians: Politician[], limit = 3): Array<Politician & { currentRank: number; rankDelta: number }> {
  return withCurrentRank(getRankedPoliticians(politicians))
    .filter((p) => p.rankDelta > 0)
    .sort((a, b) => b.rankDelta - a.rankDelta)
    .slice(0, limit);
}

export function getBiggestDrops(politicians: Politician[], limit = 3): Array<Politician & { currentRank: number; rankDelta: number }> {
  return withCurrentRank(getRankedPoliticians(politicians))
    .filter((p) => p.rankDelta < 0)
